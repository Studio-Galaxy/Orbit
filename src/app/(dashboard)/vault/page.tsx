...
import { 
  ...
  Upload, Maximize, Minimize, LayoutGrid, List
} from "lucide-react";
                 ...
                 return (
                   <motion.div layout key={file.id} className="group relative">
                     <button 
                        ...
                        className={`w-full aspect-[4/3] flex flex-col items-center justify-center p-6 rounded-[2rem] border transition-all overflow-hidden relative ${
                          isActive ? "bg-white/5 border-white/20 shadow-2xl" : "bg-neutral-900/20 border-neutral-900 hover:bg-neutral-900/40 hover:border-neutral-800"
                        }`}
                     >
                     ...
                     <button onClick={() => setFileToDelete(file.id)} className="absolute top-4 right-4 p-2 bg-neutral-950/80 text-neutral-600 hover:text-red-500 rounded-full opacity-0 group-hover:opacity-100 transition-all border border-neutral-800">
                       <X size={14} />
                     </button>
                   </motion.div>
                 );
      ...
      <AnimatePresence>
        {isAiModalOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
             ...
             <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative w-full max-w-3xl bg-neutral-900 border border-neutral-800 rounded-[2.5rem] overflow-hidden shadow-2xl">
                <div className="p-10">
                        ...
                        <div className="space-y-8 pb-10">
                           {aiActionTitle === "Summary" && (
                             <div className="p-8 bg-neutral-950 border border-neutral-800 rounded-3xl">
                                <p className="text-neutral-300 leading-relaxed text-lg italic">"{aiResult.summary}"</p>
                             </div>
                           )}
                             ...
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {aiResult.flashcards.map((fc: any, i: number) => (
                                  <div key={i} className="p-6 bg-neutral-950 border border-neutral-800 rounded-3xl group">
                                    <div className="text-[10px] font-black text-neutral-600 uppercase tracking-widest mb-4">Card {i+1}</div>
                                    <div className="font-bold text-white mb-4 text-base">Q: {fc.question}</div>
                                    <div className="text-neutral-500 text-sm italic">A: {fc.answer}</div>
                                  </div>
                                ))}
                             </div>
                           )}
                           <button onClick={async () => {
                              ...
                              className="w-full py-5 bg-white text-black text-xs font-black rounded-2xl hover:bg-neutral-200 transition-all uppercase tracking-widest"
                            >
                              Copy Analysis
                            </button>
                        </div>
